...
var
  ...
  array_lista_id_jogo : array [1..5] of integer = (0, 0, 0, 0, 0);
...
//  try
    ...
    for I := 0 to lvJogo.Items.Count - 1 do
      begin
        array_lista_id_jogo[i+1] := strtoint(lvJogo.Items[i].Caption);
      end;
    ...
    with query.Query1 do
      begin
        sql.Clear;
        sql.Add('insert into ENTRADAS (ID_JOGO_1, ID_JOGO_2, ID_JOGO_3, ID_JOGO_4, ID_JOGO_5, ODD_ENTRADA, DESCRICAO_ENTRADA, UNIDADE, VALOR_ENTRADA, RISCO, DATA_ENTRADA) values '
                                   + '(:pId_jogo_1, :pId_jogo_2, :pId_jogo_3, :pId_jogo_4, :pId_jogo_5, :pOdd, :pDescricao, :pUnidade, :pValor, :pRisco, :pData)');
        ...
        ParamByName('pDescricao').AsString := pDescriçãoJogo.Caption;
        ParamByName('pUnidade').AsFloat := unidade;
        ParamByName('pValor').AsFloat := strtofloat(pValorTotalEntrada.Caption);
        ParamByName('pRisco').AsFloat := risco_jogo;
...
procedure TfmlCadastrosJogos.pBtnCadastrarClick(Sender: TObject);
begin
  cadastrarEntrada;
end;
